class FullPage{
    ...
    nextSection(){
        if(this.section==this.sectionsCount-1)
            return;
        $(this.sectionsContainer.find(".section:eq("+this.section+")")).addClass("prev");
        this.section++;
        $(this.sectionsContainer.find(".section:eq("+this.section+")")).removeClass("next");
        this.sectionsContainer.css("top", this.section*(-100) + "vh");
        //this.transitionArray[(this.section)+"-"+(this.section+1)]();
        this.floatingNav.addClass(this.section%2?'bright':'dark');
        this.floatingNav.removeClass(this.section%2 == 0?'bright':'dark');
        console.log(this.sectionsCount%2?'bright':'dark');
    }

    prevSection(){
        if(this.section==0)
            return;
        $(this.sectionsContainer.find(".section:eq("+this.section+")")).addClass("next");
        this.section--;
        $(this.sectionsContainer.find(".section:eq("+this.section+")")).removeClass("prev");
        this.sectionsContainer.css("top", this.section*(-100) + "vh");
    ...
    constructor(element, duration){        
        ...
        this.floatingNav = this.fullpage.find('.floating-nav');
